...
app.post('/get_status_to_delivery', authenticateToken, (req, res) => {
    console.log(req.body)
    const user_actual = req.user;
    const rol_usuario = user_actual.rol;
    const id_order = req.body.id_order;
    ...
    if(rol_usuario == 1){
        ...
        request.post(
            ...
            "url" : "http://0.0.0.0:7007/send_status_to_client",
            "body": JSON.stringify({
                "username": user_actual.name,
                "id_order": id_order
            })
        ...
        res.json({ message: "Solamente los clientes con orden pueden solicitar el status" });
    }
});
